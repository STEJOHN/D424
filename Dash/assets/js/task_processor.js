...
export function getResolvedTasks(tasks) {
  return tasks.filter((task) => task.State === "Resolved");
}

export function countTasksByManager(tasks) {
  return tasks.reduce((acc, task) => {
    const manager = task["Assigned To"].split(" <")[0];
    acc[manager] = (acc[manager] || 0) + 1;
    return acc;
  }, {});
}

export function findMostRecentDates(tasks) {
  return tasks
    .map((task) => task["Work End Date"])
    .sort()
    .reverse();
}
